...
//    }
    ...
    @Test
    fun testOpenTextInNewActivityO() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)
        ...
        assertTrue(device.wait(Until.hasObject(By.pkg(packageName)), TIMEOUT))
    ...
    fun testOpenTextInNewActivity() {
        ...
        val textElement = device.wait(Until.findObject(By.res(packageName, "text")), TIMEOUT)
        assertNotNull("Text element not found", textElement)

        val result = textElement.text
        //assertEquals(textToSet, result)
        assertEquals("Text does not match", textToSet, result)
    }
